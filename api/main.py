...
async def get_debate_status(debate_id: str):
    ...
    try:
        ...
        if debate_id in active_debates:
            ...
            # Patch: Always provide a summary string, even if it's an error or missing
            summary = result.final_summary or "Summary not available."
            ...
            total_rounds = result.total_rounds if isinstance(result.total_rounds, int) else 0
            consensus_scores = result.consensus_evolution if isinstance(result.consensus_evolution, list) else []
            duration = result.total_duration if isinstance(result.total_duration, (int, float)) else 0.0
            status = result.final_status if isinstance(result.final_status, DebateStatus) else DebateStatus.ERROR
            return DebateStatusResponse(
                debate_id=debate_id,
                status=status,
